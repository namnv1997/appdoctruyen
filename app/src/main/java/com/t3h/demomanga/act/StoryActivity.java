...
import com.t3h.demomanga.Constant;
import com.t3h.demomanga.R;
import com.t3h.demomanga.adapter.StoryAdapter;
...
public class StoryActivity extends AppCompatActivity implements Constant, StoryAdapter.IFStrory, AdapterView.OnItemClickListener {
    private TextView tvTitle;
    private TextView tvContent;
    private ListView mLvStory;
    ...
    private void initViews() {
        ...
        mLvStory = (ListView) findViewById(R.id.lv_item_story);
        mStoryAdapter = new StoryAdapter(this);
        tvContent = (TextView) findViewById(R.id.tv_content);
        tvTitle = (TextView) findViewById(R.id.title_story);
        mLvStory.setAdapter(mStoryAdapter);
        mLvStory.setOnItemClickListener(this);
    }

    private void initComponents() {
        Intent intent = getIntent();
        // hung con thuyen
        int topic = intent.getIntExtra("TOPIC", 0); // 0 la default data
        String id = topic + "";

        switch (id) {
            case VOVA:
                tvTitle.setText("Vova");
                initVova();
                break;
            case GAME:
                tvTitle.setText("Game");
                break;
            case CONGAI:
                tvTitle.setText("Con Gái");
                break;
            case CONTRAI:
                tvTitle.setText("Con Trai");
                break;
            case GIADINH:
                tvTitle.setText("Gia Đình");
                break;
            case DANGIAN:
                tvTitle.setText("Dân Gian");
                break;
            case TIEULAM:
                tvTitle.setText("Tiếu Lâm");
                break;
            case TINHYEU:
                tvTitle.setText("Tình Yêu");
                initTinhYeu();
                break;
            default:
                break;
    ...
    private void initTinhYeu() {
        mItemStories.add(new ItemStory("Thương trẻ mồ côi", "Content"));
        mItemStories.add(new ItemStory("Không bẩn", "Content"));
        mItemStories.add(new ItemStory("Đã hai lần rồi", "Content"));
        mItemStories.add(new ItemStory("Chuần không phải chỉnh", "Content"));
        mItemStories.add(new ItemStory("Tỏa sáng trên sân khấu", "Content"));
        mItemStories.add(new ItemStory("Can đảm lên anh", "Content"));
        mItemStories.add(new ItemStory("Cũng chịu thôi", "Content"));
        mItemStories.add(new ItemStory("Cái chày", "Content"));
        mItemStories.add(new ItemStory("Phù hợp", "Content"));
        mItemStories.add(new ItemStory("Không có thời gian", "Content"));
        mItemStories.add(new ItemStory("Có hại cho sức khỏe", "Content"));
        mItemStories.add(new ItemStory("Ngăn ngừa hành vi xấu", "Content"));
        mItemStories.add(new ItemStory("Tường của em...", "Content"));
        mItemStories.add(new ItemStory("Hiệu quả hơn", "Content"));
        mCount = mItemStories.size();

    }

    private void initVova() {
    ...
    @Override
    public ItemStory getItemStory(int position) {
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
        mStoryAdapter.setCurrentPosition(position);
        mStoryAdapter.notifyDataSetChanged();

//        tvTitle.setTextSize(28);
//        tvTitle.setBackgroundColor(Color.BLUE);
//
//        tvTitle.setText(mItemStories.get(position).getmTitle());
//        mLvStory.setVisibility(View.INVISIBLE);
//        tvContent.setVisibility(View.VISIBLE);
//        tvContent.setText(mItemStories.get(position).getmContent());
